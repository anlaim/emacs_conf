...
(defun byte-compile-all-my-el-files()
  ...
  (let ((files  (all-files-under-dir-recursively (expand-file-name "~/.emacs.d/site-lisp/")  "\\.el$" nil
                                                 "\\.git\\|\\.svn\\|RCS\\|rcs\\|CVS\\|cvs\\|joseph_init.el$\\|malabar-1.5-SNAPSHOT\\b\\|\\bicicles\\b\\|joseph_init.el$\\|\\bcedet-mirror\\b\\|\\bcedet-1.1\\b\\|\\borg-mode-git\\b\\|\\bdistel\\b" t
                                                 ))
        (i 0)
        300eles)
    ...
    (joseph-byte-compile-files-outside 300eles))
  (let ((default-directory (expand-file-name "~/.emacs.d/site-lisp/distel/")))
    (shell-command "make"))
