...
(defun execute-sqlplus-special-format-sql (special-format-sql)
  (let* ((tmp-buf " *sqlplus-process*")
         (process (start-process-shell-command "execute single sql" tmp-buf "sqlplus scott/tiger"))
         result )
    (set-process-query-on-exit-flag process nil)
    (send-string process " set pagesize 3000 ; \n")
    (send-string process " set linesize 100 ; \n")
    (send-string process " set heading off ; \n")
    (send-string process  special-format-sql)
    (save-excursion
      (with-current-buffer (get-buffer tmp-buf)
        (goto-char (point-min))
        (while (< (point ) (point-max))
          (forward-line 1) (beginning-of-line)
          (when (looking-at "^(.*)$")
            (setq result (append  (car (read-from-string (match-string 0))) result)))
          (end-of-line))))
    (kill-process process)
    (kill-buffer tmp-buf)
    result))
;; (abc "  select '(\"'||table_name||'\") '   from user_tab_columns ; \n")
...
(defun sqlparse-oracle-shell-query(sql)
